...
const AMP_STYLE_LIGHTING =
  'this.visualThemeName="dark",this.lightBaseIntensities=new Map,this.scene.background=new hB(.11,.18,.28),this.scene.fog=null,this.ambientLight=new kh(16777215,.18),this.ambientLight.name="AmbientLight",this.scene.add(this.ambientLight),this.sonicHemi=new RR(6979754,2769450,.4),this.sonicHemi.name="SonicStyleHemisphere",this.scene.add(this.sonicHemi),this.sonicFill=new uw(8443903,.48),this.sonicFill.name="SonicStyleFill",this.sonicFill.position.set(-3,4,2),this.scene.add(this.sonicFill),this.sonicRim=new uw(16777215,.36),this.sonicRim.name="SonicStyleRim",this.sonicRim.position.set(-2,2.5,4),this.scene.add(this.sonicRim)';

const PREVIOUS_AMP_STYLE_LIGHTING =
...
} else if (bundle.includes(PREVIOUS_AMP_STYLE_LIGHTING)) {
  bundle = bundle.replace(PREVIOUS_AMP_STYLE_LIGHTING, AMP_STYLE_LIGHTING);
} else if (!bundle.includes('this.sonicHemi=new RR(6979754,2769450,.4)')) {
